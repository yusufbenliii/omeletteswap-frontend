...
export class MockV1Pair extends Pair {
  constructor(etherAmount: BigintIsh, tokenAmount: TokenAmount) {
    super(tokenAmount, new TokenAmount(WOMC[tokenAmount.token.chainId], etherAmount))
  }
}
...
function useMockV1Pair(inputCurrency?: Currency): MockV1Pair | undefined {
  ...
  const isWETH = Boolean(token && token.equals(WOMC[token.chainId]))
...
export function useV1Trade(
  ...
  const inputIsETH = inputCurrency === OMC
  const outputIsETH = outputCurrency === OMC
